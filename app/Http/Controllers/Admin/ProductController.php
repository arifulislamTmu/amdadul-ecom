...
class ProductController extends Controller
{
  ...
  public function productAdded(Request $request)
  {
    ...
    if ($request->file('product_img_four') != '') {
      ...
      \Image::make($image_four)
        ->resize(2070, 2070)
        ->save('frotend/img/product/upload/' . $name_gena4);
    ...
    if ($request->file('product_img_five') != '') {
      ...
      \Image::make($image_five)
        ->resize(2070, 2070)
        ->save('frotend/img/product/upload/' . $name_gena5);
    ...
    if ($request->file('product_img_six') != '') {
      ...
      \Image::make($image_six)
        ->resize(2070, 2070)
        ->save('frotend/img/product/upload/' . $name_gena6);
    ...
    Product::insert([
      ...
      'product_order_qty' => $request->product_order_qty,
  ...
  public function productedit($id)
  {
    $categories = Category::latest()->get();
    $brands = Brand::latest()->get();
    $product_edit = DB::table('products')->select('products.*')->join('brands', 'brands.id', '=', 'products.brand_name')->join('categories', 'categories.id', '=', 'products.category_name')->where('products.id', $id)->get();
  ...
  public function priceDiscountAdd(Request $request)
  {
      $product = Product::findOrFail($request->product_id);
     $product->product_price = $request->discout_price;
     $product->product_price_old = $request->product_price_old;
     $product->discout = $request->discout;
     $product->deal_type = "today_deals";
     $product->save();
     return redirect()->route('product.list')->with('success', 'Product Successfuly Updated.');
  }
